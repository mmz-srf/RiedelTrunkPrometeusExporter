...
- **`artist_node_up{name=...}`** (Gauge) - Verbindungsstatus je Artist-Node, aus `Connected successfully` / `Could not connect ... timed out` / `Error connecting the socket ... refused`. Der Standortname kommt ausschliesslich aus `trunk_navigator.node_names` in `config.json` (IP -> Name); die Redundanz-IP (primäre IP, letztes Oktett + 1) wird automatisch demselben Namen zugeordnet. Nicht eingetragene IPs erscheinen als rohe IP-Adresse (Wert bleibt korrekt).
